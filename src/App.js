...
console.log(imagesList[Math.ceil(Math.random() * imagesList.length)].imageUrl)
...
class App extends Component {
  ...
  decreaseSecondsCount = () => {
    ...
    if (timer === 0) {
      clearInterval(this.intervalId)
      this.setState({timer: 0, isMatch: false})
    }
  }
  ...
  getFilteredThumbnails = () => {
    ...
    const filteredThumbnails = imagesList.filter(
      eachThumbnail => activeTabId === eachThumbnail.category,
    )
  ...
  getRandomImage = id => {
    ...
    if (image.id === id) {
      this.setState({
        image: imagesList[Math.ceil(Math.random() * imagesList.length)],
      })
  ...
  render() {
    const {score, timer, image, isMatch} = this.state
    ...
    return (
      <>
        ...
        <div className="match-game-container">
          {isMatch ? (
            <>
              <img src={image.imageUrl} alt="match" className="match-image" />
              ...
              <ul className="thumbnails-item-card">
